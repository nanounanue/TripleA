...
public class ProUtils
{
	...
	public static Territory landAttackMap(final GameData data, final PlayerID player, final HashMap<Territory, Float> enemyMap)
	{
		...
		final List<Territory> enemyTerrs = ProUtils.allEnemyTerritories(data, player);
		...
		while (eTIter.hasNext())
		{
			...
			{
				final float eStrength = ProUtils.strength(eTerr.getUnits().getMatches(Matches.enemyUnit(player, data)), true, false, true);
				enemyMap.put(eTerr, eStrength);
			}
		}
		ProUtils.reorder(enemyTerrs, enemyMap, true);
		if (enemyTerrs.isEmpty())
		{
			return null;
		}
	...
	public static void continentAlliedUnitTerr(final GameData data, final PlayerID player, final Territory startTerr, final List<Territory> contiguousTerr, final List<Territory> ignoreTerr)
	{
		...
		for (final Territory n1 : neighbor1)
		{
			if (Matches.isTerritoryAllied(player, data).match(n1))
				...
				ignoreTerr.add(n1);
			ProUtils.continentAlliedUnitTerr(data, player, n1, contiguousTerr, ignoreTerr);
		}
	...
	public static float strengthOnContinent(final GameData data, final PlayerID player, final Territory continentTerr)
	{
		...
		final boolean island = !ProUtils.doesLandExistAt(continentTerr, data, false); // just make sure this is really a "continent"
		if (island)
		{
			if (Matches.isTerritoryAllied(player, data).match(continentTerr))
			{
				continentStrength += ProUtils.strength(continentTerr.getUnits().getMatches(Matches.alliedUnit(player, data)), true, false, false);
		...
		ProUtils.continentAlliedUnitTerr(data, player, continentTerr, allContinentTerr, ignoreTerr);
		for (final Territory cTerr : allContinentTerr)
			continentStrength += ProUtils.strength(cTerr.getUnits().getMatches(Matches.alliedUnit(player, data)), true, false, false);
		return continentStrength;
	}
	...
	public static void removeUnthreatenedTerritories(final GameData data, final PlayerID player, final List<Territory> terrList)
	{
		...
		while (tIter.hasNext())
		{
			...
			{
				final float eStrength = ProUtils.getStrengthOfPotentialAttackers(checkTerr, data, player, false, false, null);
	...
	public static IntegerMap<Territory> targetTerritories(final GameData data, final PlayerID player, final int tDistance)
	{
		...
		final List<Territory> playerFactories = ProUtils.findUnitTerr(data, player, enemyFactory);
		...
		for (final Territory mT : playerFactories)
		{
			...
			for (final Territory checkTerr : initialGroup)
			{
				if (Matches.isTerritoryEnemy(player, data).match(checkTerr) && Matches.TerritoryIsLand.match(checkTerr) && Matches.TerritoryIsNotImpassable.match(checkTerr))
				{
					...
					if (cDist < checkDist || (cDist >= checkDist && !ProUtils.doesLandExistAt(checkTerr, data, false)))
					{
				...
				final HashMap<Integer, IntegerMap<Territory>> unitCountList, final PlayerID player, final GameData data, final boolean tFirst)
	{
		...
		int bestFit = -1;
		float maxScore = 0.0F;
		final List<Territory> enemyCaps = ProUtils.getEnemyCapitals(data, player);
		final List<Territory> enemyFactories = ProUtils.findUnitTerr(data, player, enemyFactory);
		...
		*/
		final List<Territory> allEnemyTerr = ProUtils.allEnemyTerritories(data, player);
		...
		for (Integer i = 0; i <= numMoves - 1; i++)
		{
			...
			for (final Territory eTerr : allEnemyTerr)
			{
				...
				{
					eNeighbors.addAll(ProUtils.getExactNeighbors(eTerr, j, player, data, false));
	...
	public static IntegerMap<PlayerID> getPlayerTUV(final GameData data)
	{
		...
		for (final Territory allTerr : data.getMap().getTerritories())
		{
			for (final PlayerID onePlayer : playerList)
			{
				...
				final IntegerMap<UnitType> uMap = ProUtils.convertListToMap(playerUnits);
				int tuv = TUVMap.getInt(onePlayer);
				tuv += ProUtils.determineTUV(uMap, costMap.get(onePlayer));
				...
				final PlayerID player, final boolean tFirst)
	{
		...
		final List<Territory> ignoreTerr = new ArrayList<Territory>();
		ProUtils.continentAlliedUnitTerr(data, player, targetTerr, contiguousTerritories, ignoreTerr);
		...
		ProUtils.reorder(contiguousTerritories, distanceMap, false);
		ProUtils.trimTerritoryList(contiguousTerritories, 3); // look at the top 3
		...
		if (goTerr == null)
		{
			ProUtils.reorder(contiguousTerritories, unitMap, true);
			if (!contiguousTerritories.isEmpty())
	...
	{
		final List<Territory> eCapitols = ProUtils.getEnemyCapitals(data, player);
		int maxDistance = 100;
				...
				final HashMap<PlayerID, IntegerMap<UnitType>> costMap, final PlayerID player, final GameData data, final boolean aggressive, final boolean subRestricted, final boolean tFirst)
	{
		...
		if (weWin && (eTUV == 0 || (((ProUtils.strength(defenderUnits, false, eTerr.isWater(), tFirst) * 5F) + 10F) < ProUtils.strength(invasionUnits, true, eTerr.isWater(), tFirst))))
	...
	public static List<Territory> getTerritoriesWithEnemyNeighbor(final GameData data, final PlayerID player, final boolean allied, final boolean neutral)
	{
		...
		final List<Territory> enemyLandTerr = ProUtils.allEnemyTerritories(data, player);
		if (!neutral)
		{
		...
		while (eIter.hasNext())
		{
			...
			if (ProUtils.doesLandExistAt(enemy, data, false))
			{
				final List<Territory> newTerrs = new ArrayList<Territory>();
				if (allied)
					newTerrs.addAll(ProUtils.getNeighboringLandTerritories(data, player, enemy));
				else
				...
				final boolean tFirst)
	{
		...
		for (final Territory xWaterTerr : waterTerr)
		{
			...
			eStrength -= ProUtils.strength(xWaterTerr.getUnits().getMatches(alliedSeaUnit), false, true, tFirst);
	...
	public static List<Territory> possibleBlitzTerritories(final Territory checkTerr, final GameData data, final PlayerID player)
	{
		...
		final List<Territory> twoTerr = ProUtils.getExactNeighbors(checkTerr, 2, player, data, false);
		final List<Territory> oneTerr = ProUtils.getExactNeighbors(checkTerr, 1, player, data, false);
		for (final Territory blitzFrom : twoTerr)
		{
			...
			if (blitzUnits.isEmpty())
				continue;
			final List<Territory> blitzNeighbors = ProUtils.getExactNeighbors(blitzFrom, 1, player, data, false);
	...
	*/
	...
	{ // generate a list of all enemy capitals
		...
		final List<PlayerID> ePlayers = ProUtils.getEnemyPlayers(data, player);
				...
				final List<Territory> ignoreTerr)
	{
		...
		while (playerIter.hasNext())
		{
			...
			final List<Territory> eFTerrs = ProUtils.findUnitTerr(data, ePlayer, enemyPlane);
			...
			if (maxBomberDistance < 0)
				maxBomberDistance = 0;
			final List<Territory> eTTerrs = ProUtils.findUnitTerr(data, ePlayer, aTransport);
				...
				for (final Territory t4 : data.getMap().getNeighbors(location, maxTransportDistance))
				{
					...
					while (!transportsCounted && iterTerr.hasNext())
					{
						...
						for (final Territory xN : transNeighbors)
						{
							...
							final List<Unit> availTransUnits = ProUtils.sortTransportUnits(aTransUnits);
				...
				final List<Territory> checked)
	{
		...
		while (!q.isEmpty())
		{
			...
			if (distance.getInt(current) == maxDistance)
				break;
			for (final Territory neighbor : data.getMap().getNeighbors(current, ProUtils.TerritoryIsNotImpassableToAirUnits(data)))
			{
				if (!distance.keySet().contains(neighbor))
				{
					q.add(neighbor);
	...
	*/
	...
	public static boolean airUnitIsLandableOnCarrier(final Unit u, final Territory source, final Territory target, final PlayerID player, final GameData data)
	{
		...
		final int rDist = data.getMap().getDistance(source, target, ProUtils.TerritoryIsNotImpassableToAirUnits(data));
		boolean landable = false;
		if (MoveValidator.hasEnoughMovement(u, rDist))
		{
			final Route r = findNearest(target, condition, ProUtils.TerritoryIsNotImpassableToAirUnits(data), data);
			if (r == null)
				return false;
	...
	public static Territory findFactoryTerritory(final GameData data, final PlayerID player, final float risk, boolean buyfactory, final boolean onWater)
	{
		...
		final List<Territory> owned = allOurTerritories(data, player);
		final List<Territory> existingFactories = ProUtils.findTersWithUnitsMatching(data, player, Matches.UnitCanProduceUnits); // TODO: maybe use Matches.UnitCanProduceUnitsAndIsConstruction
		owned.removeAll(existingFactories);
		final List<Territory> isWaterConvoy = ProUtils.onlyWaterTerr(data, owned);
		...
		if (onWater)
		{
			final List<Territory> waterOwned = ProUtils.stripLandLockedTerr(data, owned);
			...
			for (final Territory prodTerr : owned)
			{
				...
				final List<Territory> isWater = ProUtils.onlyWaterTerr(data, weOwnAll);
			...
			ProUtils.reorder(owned, terrProd, true);
		...
		for (final Territory t : owned)
		{
			...
			final List<Territory> closeAllies = ProUtils.getNeighboringLandTerritories(data, player, t);
			...
			{
				tStrength += ProUtils.strength(cA.getUnits().getMatches(Matches.alliedUnit(player, data)), false, false, false);
			}
	...
	public static Territory findASeaTerritoryToPlaceOn(final Territory landTerr, final GameData data, final PlayerID player, final boolean tFirst)
	{
		...
		for (final Territory t : seaNeighbors) // give preference to territory with units
		{
			...
			final float extraEnemy = ProUtils.strength(t.getUnits().getMatches(Matches.enemyUnit(player, data)), true, true, tFirst);
				...
				final List<Route> moveRoutes, final GameData data, final PlayerID player, final boolean attacking, final boolean tFirst, final boolean includeTransports, final Match<Unit> types)
	{
		...
		final List<Territory> possibleShipTerr = ProUtils.findTersWithUnitsMatching(data, player, ownedSeaUnit);
	...
	public static float strengthOfTerritory(final GameData data, final Territory thisTerr, final PlayerID player, final boolean attacking, final boolean sea, final boolean tFirst, final boolean allied)
	{
		...
		final float theStrength = ProUtils.strength(theUnits, attacking, sea, tFirst);
		return theStrength;
	}
	...
	public static float verifyPlaneAttack(final GameData data, final List<Collection<Unit>> xMoveUnits, final List<Route> xMoveRoutes, final PlayerID player, final List<Territory> alreadyAttacked)
	{
		...
		for (final Territory alliedTerr : ProUtils.allAlliedTerritories(data, player))
		{
			final float eStrength = ProUtils.getStrengthOfPotentialAttackers(alliedTerr, data, player, false, false, alreadyAttacked);
			float ourStrength = ProUtils.strengthOfTerritory(data, alliedTerr, player, false, false, false, true);
		...
		while (xMoveIter.hasNext())
		{
			...
			while (xMIter.hasNext())
			{
				...
				if (Matches.UnitIsAir.match(plane))
				{
					...
					ProUtils.reorder(endNeighbors, strengthDiffMap, false);
					...
					while (eIter.hasNext() && !safePlane)
					{
						...
						{
							final float strengthDiff = strengthDiffMap.get(newTerr) - ProUtils.uStrength(plane, false, false, false);
		...
		for (final Territory checkTerr : badMoveTerrs)
		{
			...
			for (final Integer killRoute : routeNumber)
			{
				...
				strengthEliminated += ProUtils.strength(killUnits, true, false, false);
	...
	public static boolean territoryHasThreatenedAlliedFactoryNeighbor(final GameData data, final Territory eTerr, final PlayerID player)
	{
		...
		{
			float eStrength = ProUtils.getStrengthOfPotentialAttackers(factory, data, player, false, true, null);
			eStrength += eStrength * 1.15F + (eStrength > 2.0F ? 3.0F : 0.0F);
			float myStrength = ProUtils.strength(factory.getUnits().getUnits(), false, false, false);
			if (eStrength > myStrength)
			{
				...
				{
					addStrength += ProUtils.strengthOfTerritory(data, fNTerr, player, false, false, false, true);
				}
				...
				final PlayerID player, final boolean tFirst, final boolean waterBased, final boolean nonCombat)
	{
		...
		final List<Territory> enemyCapitals = ProUtils.getEnemyCapitals(data, player);
		int minDist = 1000;
		 ...
		 */
		final List<Territory> alliedFactories = ProUtils.getEnemyCapitals(data, ePlayer);
		final Iterator<Territory> aFIter = alliedFactories.iterator();
		while (aFIter.hasNext())
		{
			final Territory aFTerr = aFIter.next();
			final float aFPotential = ProUtils.getStrengthOfPotentialAttackers(aFTerr, data, player, tFirst, true, null);
			final float alliedStrength = ProUtils.strengthOfTerritory(data, aFTerr, player, false, false, tFirst, true);
		...
		for (final Territory eTerr : data.getMap().getTerritories())
		{
			...
			final boolean island = !ProUtils.doesLandExistAt(eTerr, data, false);
	...
	{
		final List<Territory> blitzers = ProUtils.possibleBlitzTerritories(eTerr, data, player);
		...
		while (bIter.hasNext())
		{
			...
			if (!blitzCounted && Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data).match(newBTerr))
				nonBlitzStrength += ProUtils.strength(newBTerr.getUnits().getMatches(Matches.enemyUnit(player, data)), true, false, false);
		}
	...
	public static HashMap<Territory, Float> rankAmphibReinforcementTerritories(final GameData data, final List<Territory> ignoreTerr, final PlayerID player, final boolean tFirst)
	{
		...
		for (final Territory aTerr : data.getMap().getTerritories())
		{
			...
			final float localStrength = ProUtils.strength(aTerr.getUnits().getUnits(), false, false, tFirst);
			...
			if (Matches.territoryHasAlliedNeighborWithAlliedUnitMatching(data, player, Matches.UnitCanProduceUnits).match(capTerr)) // does this need reinforcing?
			{
				...
				{
					final float xValue = ProUtils.twoAwayStrengthNotCounted(data, player, aTerr);
