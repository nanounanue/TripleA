...
public class RevisedTest extends TestCase
{
	...
	public void testAlliedNeighbors()
	{
		...
		final Set<Territory> enemyNeighbors = m_data.getMap().getNeighbors(centralUs, Matches.isTerritoryEnemy(americans, m_data));
	...
	public void testContinuedBattles()
	{
		...
		final TripleAUnit sub = (TripleAUnit) Match.getMatches(sz5.getUnits().getUnits(), Matches.unitIsOwnedBy(russians)).iterator().next();
		sub.setSubmerged(true);
		// now move an infantry through the sz
		String results = moveDelegate.move(Match.getNMatches(germany.getUnits().getUnits(), 1, Matches.unitIsOfType(infantryType)), m_data.getMap().getRoute(germany, sz5),
	...
	public void testLoadAlliedTransports()
	{
		...
		final String error = moveDelegate(m_data).move(uk.getUnits().getMatches(Matches.unitIsOwnedBy(americans)), new Route(uk, sz2), sz2.getUnits().getMatches(Matches.UnitIsTransport));
	...
	public void testLoadUnloadAlliedTransport()
	{
		...
		eeToSz5.add(sz5);
		// load the transport in the baltic
		final List<Unit> infantry = eastEurope.getUnits().getMatches(new CompositeMatchAnd<Unit>(Matches.unitIsOfType(infantryType), Matches.unitIsOwnedBy(japanese)));
	...
	public void testMoveSubAwayFromSubmergedSubsInBattleZone()
	{
		...
		sz45To50.add(sz50);
		final List<Unit> japSub = sz45.getUnits().getMatches(new CompositeMatchAnd<Unit>(Matches.UnitIsSub, Matches.unitIsOwnedBy(japanese)));
	...
	public void testAAOwnership()
	{
		...
		// Set up battle
		MustFightBattle battle = (MustFightBattle) AbstractMoveDelegate.getBattleTracker(m_data).getPendingBattle(fic, false, null);
	...
	public void testAttackSubsOnDestroyer()
	{
		 ...
		 * 
		 * Every sub, regardless of whether it is on the attacking or defending side, fires in the Opening Fire step of combat. That is the only time a sub ever fires.
		 ...
		 * If the enemy (attacker or defender) has a destroyer, then hits caused by your subs are not removed until the Remove Casualties step (step 6) of combat.
		 * 
	...
	public void testTransportsUnloadingToMultipleTerritoriesDie()
	{
		...
		// fight the battle
		moveDelegate(m_data).end();
		final MustFightBattle battle = (MustFightBattle) AbstractMoveDelegate.getBattleTracker(m_data).getPendingBattle(sz6, false, null);
